...
import { logError, changeSpinner, logFailure, logVerbose, logMessage } from "./log.js";
...
export async function debugIsolateBundlesSerially(
  ...
  for (const entryPoint of entryPoints) {
    changeSpinner(`bundling entry point ${entryPoint} (${i++}/${entryPoints.length})...`);
    ...
    try {
      ...
      logError("");
      logError(`Bundling ${entryPoint} resulted in ${error.errors.length} esbuild errors.`);
