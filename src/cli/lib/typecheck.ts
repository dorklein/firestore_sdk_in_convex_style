...
export async function typeCheckFunctionsInMode(
  ...
  await typeCheckFunctions(ctx, functionsDir, async (result, logSpecificError, runOnError) => {
    if (
      (result === "cantTypeCheck" && typeCheckMode === "enable") ||
      result === "typecheckFailed"
    ) {
      logSpecificError?.();
      logError(chalk.gray("To ignore failing typecheck, use `--typecheck=disable`."));
      try {
...
async function runTsc(
  ...
  if (!ctx.fs.exists(tscPath)) {
    return handleResult("cantTypeCheck", () => {
      logError(chalk.gray("No TypeScript binary found, so skipping typecheck."));
    });
  }

  // Check the TypeScript version matches the recommendation from Convex
  const versionResult = await spawnAsync(ctx, process.execPath, [tscPath, "--version"]);
...
async function runTscInner(
  ...
  const result = await spawnAsync(ctx, process.execPath, [tscPath, ...tscArgs, "--listFiles"]);
  ...
  return handleResult(
    ...
    async () => {
      ...
      await spawnAsync(ctx, process.execPath, [tscPath, ...tscArgs, "--pretty", "true"], {
        stdio: "inherit",
      });
