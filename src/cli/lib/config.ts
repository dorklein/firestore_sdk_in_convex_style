...
import { logError, logFailure, logFinishedStep, logMessage } from "../../bundler/log.js";
import { Bundle, BundleHash } from "../../bundler/index.js";
import { formatSize, functionsDir, loadPackageJson } from "./utils/utils.js";
...
//   }
//   const convexResult = await bundle(ctx, baseDir, entryPoints.isolate, true, "browser");
//   if (verbose) {
//     logMessage(
...
//         [...nodeResult.externalDependencies.entries()].map((a) => `${a[0]}: ${a[1]}`)
//       );
//     }
//   }
...
//     ...Array.from(nodeResult.bundledModuleNames.keys()).map((moduleName): BundledModuleInfo => {
//       return {
//         name: moduleName,
//         platform: "node",
//       };
...
//     logMessage(chalk.yellowBright(`Deleted ${configPath} since it completely matched defaults`));
//   }
...
//       changeSpinner("Installing external packages and deploying source code...");
//     } else {
...
function renderModule(module: { path: string; sourceMapSize: number; sourceSize: number }): string {
...
function compareModules(
  ...
  let diff = "";
  const oldModuleMap = new Map(oldModules.map((value) => [value.path, value.hash]));
...
//       const result = await ensureWorkosEnvironmentProvisioned(ctx, deploymentName, deployment, {
...
//     const envVarMessage =
//       `Environment variable ${chalk.bold(variableName)} is used in auth config file but ` +
//       `its value was not set.`;
...
//       const variableQuery = variableName !== undefined ? `?var=${variableName}` : "";
