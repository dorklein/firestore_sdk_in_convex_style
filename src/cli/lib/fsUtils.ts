...
export async function recursivelyCopy(ctx: Context, nodeFs: NodeFs, src: string, dest: string) {
  const st = nodeFs.stat(src);
  if (st.isDirectory()) {
    nodeFs.mkdir(dest, { recursive: true });
    for (const entry of nodeFs.listDir(src)) {
      await recursivelyCopy(ctx, nodeFs, path.join(src, entry.name), path.join(dest, entry.name));
    }
  } else {
